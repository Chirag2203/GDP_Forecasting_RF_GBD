...
# Streamlit deployment layout setup

st.title('Country GDP Estimation Model-')
st.write('''
         This model uses Random Forest Regressor and Gradient boosting Regressor to estimate the GDP of
         a country based on its attributes.
         
         Fill the input attributes and click on the 'Predict' button to
         get the estimated GDP.
         ''')

st.header('Input Attributes')
att_popl = st.number_input('Population (Example: 9000000)', min_value=1e4, max_value=2e9, value=1e7)
att_area = st.slider('Area (sq. Km)', min_value= 2.0, max_value= 17e6, value=6e5, step=1e4)
att_dens = st.slider('Population Density (per sq. mile)', min_value= 0, max_value= 12000, value=100, step=10)
...
att_mort = st.slider('Infant mortality (per 1000 births)', min_value= 0, max_value=195, value=10, step=5)
att_litr = st.slider('Population literacy Percentage', min_value= 0, max_value= 100, value=10, step=10)
att_phon = st.slider('Phones per 1000', min_value= 0, max_value= 1000, value=10, step=25)
att_arab = st.slider('Arable Land (%)', min_value= 0, max_value= 100, value=10, step=2)
att_crop = st.slider('Crops Land (%)', min_value= 0, max_value= 100, value=10, step=2)
att_othr = st.slider('Other Land (%)', min_value= 0, max_value= 100, value=10, step=2)
...
att_brth = st.slider('Annual Birth Rate (births/1,000)', min_value= 7, max_value= 50, value=10, step=2)
att_deth = st.slider('Annual Death Rate (deaths/1,000)', min_value= 2, max_value= 30, value=10, step=2)
att_agrc = st.slider('Agricultural Economy', min_value= 0.0, max_value= 1.0, value=0.10, step=0.05)
att_inds = st.slider('Industrial Economy', min_value= 0.0, max_value= 1.0, value=0.10, step=0.05)
att_serv = st.slider('Services Economy', min_value= 0.0, max_value= 1.0, value=0.10, step=0.05)
st.text('(Agricultural, Industrial, and Services Economy should add up to 1 as they are in %)')
...
#Implementing a Random Forest model and GBM model
#import dataset
def get_dataset():
...
if st.button('Estimate GDP'):
    ...
    rf1_pred = rf1.predict(user_input) 
    gbm_predictions = gbm_opt.predict(user_input) 
    
    st.write('The estimated GDP per capita using Random Forest Regression Model: ', rf1_pred)
    st.write('The estimated GDP per capita using GBM: ', gbm_predictions)
